...
class PageIndexRAG:
    ...
    def query(self, query: str) -> RetrievalOutput:
        ...
        try:
            ...
            if reasoning_trajectory is not None:
                reasoning_trajectory["document_selection"].update(
                    {
                        "source": "llm",
                        "catalog_source": parsed.get("catalog_source", ""),
        ...
        for item in document_selections:
            if not isinstance(item, dict):
                continue
            doc_id = self._resolve_document_id(str(item.get("document_id", "")))
    ...
    def _select_documents(self, query: str) -> tuple[list[dict[str, str]], dict[str, Any]]:
        ...
        catalog_source = "all_documents"
        if catalog_text.endswith("\n...TRUNCATED..."):
            max_candidates = max(
                1, int(self.cfg.get("max_document_catalog_candidates", 80))
            )
            candidate_doc_ids = self._rank_documents_for_query(query)[:max_candidates]
            if candidate_doc_ids:
                catalog_text = self._format_document_catalog(
                    max_chars=max_catalog_chars,
                    doc_ids=candidate_doc_ids,
                )
                catalog_source = "query_ranked"
        system = (
    ...
    def _format_document_catalog(
        self, *, max_chars: int, doc_ids: list[str] | None = None
    ) -> str:
        lines = []
        items = (
            [(doc_id, self.trees[doc_id]) for doc_id in doc_ids if doc_id in self.trees]
            if doc_ids is not None
            else sorted(self.trees.items())
        )
        for doc_id, tree in items:
            summary = _preview(
    ...
    def _rank_documents_for_query(self, query: str) -> list[str]:
        ...
        for doc_id, tree in self.trees.items():
            ...
            score = (
                _term_match_score(terms, doc_id) * 4
                + _term_match_score(terms, str(tree.get("title", ""))) * 3
                + _term_match_score(terms, titles) * 2
                + _term_match_score(terms, str(tree.get("summary", "")))
            )
            if score:
    ...
    def _resolve_document_id(self, value: str) -> str:
        ...
        if requested in self.trees:
            return requested
        prefixed = f"cuad/{requested}"
        if prefixed in self.trees:
            return prefixed

        requested_key = _document_id_key(requested)
        if not requested_key:
            return ""
        matches = []
        for doc_id in self.trees:
            variants = {
                doc_id,
                doc_id.removeprefix("cuad/"),
                Path(doc_id).name,
                Path(doc_id).stem,
            }
            if requested_key in {_document_id_key(variant) for variant in variants}:
    ...
    def _keyword_document_fallback(self, query: str) -> list[dict[str, str]]:
        return [
            ...
            for doc_id in self._rank_documents_for_query(query)[
                : max(1, int(self.cfg.get("selected_documents", 3)))
            ]
        ]
...
def _document_id_key(value: str) -> str:
    return re.sub(r"[^a-z0-9]+", "", value.casefold())


def _term_match_score(terms: set[str], text: str) -> int:
    lowered = text.casefold()
    compact = _document_id_key(text)
    score = 0
    for term in terms:
        if term in lowered:
            score += 2
