...
def _resolve_repo_root(cfg: dict[str, Any], *, cache_dir: Path) -> Path:
    ...
    if not bool(cfg.get("auto_clone_repo", True)):
        raise RuntimeError(
            "Official PageIndex repo not found. Set GRID_PAGEINDEX_REPO_PATH "
            "or PAGEINDEX_REPO_PATH, or enable GRID_PAGEINDEX_AUTO_CLONE_REPO."
        )
...
def _runtime_package_name(repo_root: Path) -> str:
    ...
    return f"grid_agent_core.rag_compat.official_pageindex._official_runtime_{suffix}"
